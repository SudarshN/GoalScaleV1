 ...
 */
public class GoalQueryForm {

    private static final Logger LOG = Logger.getLogger(GoalQueryForm.class.getName());
     ...
     */
    public GoalQueryForm filter(Filter filter) {
        if (filter.operator.isInequalityFilter()) {
     ...
     */
    @ApiResourceProperty(ignored = AnnotationBoolean.TRUE)
    public Query<Goal> getQuery() {
        // First check the feasibility of inequality filters.
        checkFilters();
        Query<Goal> query = ofy().load().type(Goal.class);
